<template>
  <a-form v-bind="formItemLayout" :form="form" autocomplete="off">
    ...
    <a-form-item label="公司总人数">
      <a-input-number
        v-decorator="field.numberOfCompany.dec"
        v-bind="field.numberOfCompany.other"
      />
      <span :style="{ marginLeft: '5px' }">个</span>
    </a-form-item>
    <a-form-item label="清收团队数">
      <a-input-number
        v-decorator="field.teamNum.dec"
        v-bind="field.teamNum.other"
      />
      <span :style="{ marginLeft: '5px' }">个</span>
    </a-form-item>
    <a-form-item label="清收团队总人数">
      <a-input-number
        v-decorator="field.teamNumTotal.dec"
        v-bind="field.teamNumTotal.other"
      />
      <span :style="{ marginLeft: '5px' }">个</span>
    </a-form-item>
    <a-form-item label="组织架构">
      <a-textarea
        v-decorator="field.framework.dec"
        v-bind="field.framework.other"
      />
    </a-form-item>
    <a-form-item label="所控制的其他主体">
      <a-input
        v-decorator="field.otherMasterSubject.dec"
        v-bind="field.otherMasterSubject.other"
      />
    </a-form-item>
    <div class="factor-form-subtitle"><span>擅长优势</span></div>
    <a-form-item label="擅长业务类型" class="form-item-row">
      <a-checkbox-group
        v-decorator="adv.goodCases.dec"
        v-bind="adv.goodCases.other"
      >
        <a-row>
          <a-col
            v-for="item in adv.goodCases.options"
            :key="item.id"
    ...
    <a-form-item label="擅长业务区域">
      <el-cascader
        v-bind="adv.involve.other"
        @change="(val) => handleEleCas(val, adv.involve.dec[0])"
        @visible-change="(val) => (this.visible = val)"
      />
    ...
    <a-form-item label="社会资源优势">
      <a-textarea
        v-decorator="adv.otherResourcesAdvantage.dec"
        v-bind="adv.otherResourcesAdvantage.other"
      />
    </a-form-item>
    <a-form-item label="是否有投行项目经验" class="form-item-row">
      <a-radio-group
        v-decorator="adv.isHasBankExperience.dec"
        v-bind="adv.isHasBankExperience.other"
      >
        <a-row>
          <a-col :span="6">
    ...
    <a-form-item label="历史投行项目案例">
      <a-input
        v-decorator="adv.investmentBankProjectCase.dec"
        v-bind="adv.investmentBankProjectCase.other"
      />
    </a-form-item>
    ...
    >
      <a-radio-group
        v-decorator="intention.hasInvestmentIntention.dec"
    ...
    <template
      v-if="
        isFirst
          ? getValue(intention.hasInvestmentIntention.dec[0]) === '1'
    ...
    >
      ...
      >
        <a-checkbox-group
          v-decorator="intention.investmentPreferenceType.dec"
          v-bind="intention.investmentPreferenceType.other"
        >
          <a-row>
            <a-col
              v-for="item in intention.investmentPreferenceType.options"
              :key="item.id"
      ...
      <a-form-item label="投资区域">
        <a-cascader
          v-decorator="intention.investmentArea.dec"
          v-bind="intention.investmentArea.other"
        />
      </a-form-item>
      <a-form-item label="以往投资经历">
        <a-textarea
          v-decorator="intention.investmentExperience.dec"
          v-bind="intention.investmentExperience.other"
        />
      </a-form-item>
    </template>
  </a-form>
    ...
    return {
      ...
      field: {
        ...
        local: {
          dec: [
            "headOfficeAddress",
            { rules: [{ required: true, message: "请选择公司总部所在地!" }] },
          ],
          other: {
            options: areaOption,
        ...
        teamNum: {
          dec: [
            "numberOfTeams",
            { rules: [{ required: true, message: "清收团队数不能为空" }] },
          ],
          other: {
        ...
        numberOfCompany: {
          dec: [
            "numberOfCompany",
            { rules: [{ required: true, message: "公司总人数不能为空" }] },
          ],
          other: {
        ...
        teamNumTotal: {
          dec: [
            "totalTeamSize",
            { rules: [{ required: true, message: "清收团队总数不能为空" }] },
          ],
          other: {
        ...
        otherMasterSubject: {
          ...
          other: {
            placeholder:
              "请输入曾使用并参与过不良资产业务的其他主体，若有多个请用中文顿号隔开",
            ...textarea,
          },
        },
      },
      intention: {
        hasInvestmentIntention: {
          dec: [
            "hasInvestmentIntention",
            { rules: [{ required: true, message: "请选择投资意向" }] },
          ],
          other: {
            ...baseWidth,
          },
        ...
        investmentPreferenceType: {
          dec: [
            "investmentPreferenceType",
            { rules: [{ required: true, message: "请选择投资偏好类型!" }] },
          ],
          options: hisCoo,
          other: {
            ...baseWidth,
        ...
        investmentArea: {
          dec: [
            "investmentArea",
            { rules: [{ required: true, message: "请选择投资区域!" }] },
          ],
          other: {
            options: areaOption,
            placeholder: "请选择投资区域",
      ...
      adv: {
        goodCases: {
          dec: [
            "goodCases",
            { rules: [{ required: true, message: "擅长业务类型不能为空!" }] },
          ],
        ...
        otherResourcesAdvantage: {
          ...
          other: {
            placeholder:
              "包括但不限于某法院、法官，法官请以“xx法院-xx法官形式输入，多个法院、法官间以中文顿号隔开”",
            ...textarea,
          },
        },
        isHasBankExperience: {
  ...
  methods: {
    ...
    resetFormValue(source) {
      const fieldValues = {
        ...
        headOfficeAddress: (source.headOfficeAddress || "")
          .split(",")
          .map((i) => Number(i)),
        investmentPreferenceType: (source.investmentPreferenceType || "")
          .split(",")
          .map((i) => Number(i)),
        investmentArea: (source.investmentArea || "")
          .split(",")
          .map((i) => Number(i)),
      };
